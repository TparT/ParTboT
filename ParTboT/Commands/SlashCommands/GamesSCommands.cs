...
namespace ParTboT.Commands.SlashCommands
{
    public class GamesSCommands : SlashCommandModule
    {
        ...
        {
            [SlashCommandGroup("Play", "Play a game inside of Discord in the current channel!")]
            public class Play : SlashCommandModule
            {
        ...
        public class Stats : SlashCommandModule
        {
            ...
            (InteractionContext ctx, [Option("Username", "The name of the user to check the game stats for.")] string Username)
            {
                ...
                ApexLegendsStats Stats = await Bot.Services.TrackggClient.GetPlayerApexLegendsStats(Username, PlatformRoutes.Playstation);
            ...
            {
                await ctx.TriggerThinkingAsync().ConfigureAwait(false);
                CsGoStats Stats = await Bot.Services.TrackggClient.GetPlayerCSGOStats(Username);
